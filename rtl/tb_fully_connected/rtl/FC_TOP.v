...
generate
  ...
  begin
    ...
    begin
      ...
      begin
    	  regg[i] <= (set_output) ? psum_out[TILING_SIZE-i-1] : regg[i];
      end
    end
  end
endgenerate
wire [DATA_WIDTH-1:0] reg_1, reg_2, reg_3, reg_4;
assign reg_1 = regg[0];
assign reg_2 = regg[1];
assign reg_3 = regg[2];
assign reg_4 = regg[3];
...
FIFO_ASYNCH #(.DATA_WIDTH(IFM_WIDTH), .FIFO_SIZE(IFM_SIZE), .ADD_WIDTH(ADD_WIDTH)) ifm_buffer(
	.clk1(clk1)  ,
  .clk2(clk1)  ,
  .rd_clr(rd_ifm_clr),
  .wr_clr(wr_ifm_clr),
...
generate
  ...
  begin
    PE #(.DATA_WIDTH(DATA_WIDTH), .PSUM_WIDTH(DATA_WIDTH), .WGT_WIDTH(WGT_WIDTH), .IFM_WIDTH(IFM_WIDTH), .POOLING(0)) pe (
    		.clk(clk1)
    	 ,.rst_n(rst_n)
    	 ,.set_reg(set_reg)
    	 ,.ifm(ifm_out)
